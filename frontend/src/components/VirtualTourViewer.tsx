...
const VirtualTourViewer: React.FC<VirtualTourViewerProps> = ({ isOpen, onClose, propertyName }) => {
  const [currentView, setCurrentView] = useState(0);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [showInfo, setShowInfo] = useState(true);
  const containerRef = useRef<HTMLDivElement>(null);
  
  // Sample panoramic views for the virtual tour
    ...
    <AnimatePresence>
      ...
      >
        ...
        <div className="w-full h-full relative">
          {/* For demo purposes, using a gradient background instead of actual panoramic images */}
          <div className={`w-full h-full ${panoramicViews[currentView].gradient} flex items-center justify-center`}>
          ...
          <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-between px-4">
            <button 
              ...
              onClick={() => setCurrentView((currentView - 1 + panoramicViews.length) % panoramicViews.length)}
            >

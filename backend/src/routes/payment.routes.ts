  ...
  async (req: any, res: any) => {
    try {
      ...
      try {
        ...
        console.log(
          "DEBUG: File stored as base64 for database-first storage strategy"
        );
      } catch (e) {
...
router.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const checkInFrom = req.query.checkInFrom as string;
      const checkInTo = req.query.checkInTo as string;
      const checkOutFrom = req.query.checkOutFrom as string;
      const checkOutTo = req.query.checkOutTo as string;
...
router.get("/receipt/:filename", requireAuth(), async (req: any, res: any) => {
  try {
    ...
    console.log(
      "Database storage not available, checking file paths:",
      possiblePaths
    );
        ...
        debug:
          process.env.NODE_ENV === "development"
            ? {
                ...
                hasDbBackup: !!(payment?.gatewayResponse as any)?.receiptBackup,
              }
            : undefined,
      });
    }

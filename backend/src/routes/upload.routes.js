...
const processImage = (inputPath_1, outputPath_1, ...args_1) => __awaiter(void 0, [inputPath_1, outputPath_1, ...args_1], void 0, function* (inputPath, outputPath, options = {}) {
 ...
 * @access  Protected
 */
router.post('/images', (0, authservice_1.requireAuth)(), imageUpload.array('images', 10), (0, error_middleware_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        for (const file of req.files) {
            ...
            // Save to database
            const image = yield server_1.prisma.media.create({
                data: {
 ...
 */
router.post('/images/property/:propertyId', (0, authservice_1.requireAuth)(client_1.UserRole.ADMIN), imageUpload.array('images', 20), [
    (0, express_validator_1.param)('propertyId').isString(),
    ...
    const property = yield server_1.prisma.property.findUnique({
        where: { id: propertyId },
    });
    if (!property) {
    ...
    try {
        ...
        yield server_1.prisma.property.update({
            where: { id: propertyId },
            data: {
...
router.post('/documents', (0, authservice_1.requireAuth)(), documentUpload.array('documents', 5), [
    ...
    try {
        for (const file of req.files) {
            const document = yield server_1.prisma.media.create({
                data: {
                    fileName: file.filename,
                    originalName: file.originalname,
                    mimeType: file.mimetype,
...
router.get('/media', (0, authservice_1.requireAuth)(), [
    ...
    const mediaWithUrls = media.map(item => (Object.assign(Object.assign({}, item), { url: `/uploads/${item.type.toLowerCase()}s/${item.fileName}`, thumbnailUrl: item.thumbnailPath ? `/uploads/${item.type.toLowerCase()}s/thumb_${item.fileName}` : null, optimizedUrl: item.optimizedPath ? `/uploads/${item.type.toLowerCase()}s/opt_${item.fileName}` : null })));
    res.json({
        success: true,
        data: {
            media: mediaWithUrls,
 ...
 */
router.delete('/media/:id', (0, authservice_1.requireAuth)(), (0, error_middleware_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const media = yield server_1.prisma.media.findUnique({
    ...
    try {
        ...
        yield Promise.all(filesToDelete.map((filePath) => __awaiter(void 0, void 0, void 0, function* () {
            try {
        ...
        yield server_1.prisma.media.delete({
            where: { id: req.params.id },
        });
...
router.put('/media/:id', (0, authservice_1.requireAuth)(), [
    ...
    const updatedMedia = yield server_1.prisma.media.update({
        where: { id: req.params.id },
        data: {
            metadata: Object.assign(Object.assign({}, media.metadata), { description,
                alt,
                caption,
                isPrimary }),
        },
...
router.post('/media/reorder', (0, authservice_1.requireAuth)(), [
    ...
    (0, express_validator_1.body)('entityType').isIn(['PROPERTY', 'USER', 'BOOKING']).withMessage('Valid entity type required'),
    ...
    yield Promise.all(mediaIds.map((mediaId, index) => server_1.prisma.media.update({
        where: { id: mediaId },
        data: {
            metadata: {
                order: index,
 ...
 * @access  Protected
 */
router.get('/stats', (0, authservice_1.requireAuth)(), (0, error_middleware_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    const where = {};
    ...
    const [totalFiles, totalSize, filesByType, recentUploads,] = yield Promise.all([
        server_1.prisma.media.count({ where }),
    ...
    res.json({
        ...
        data: {
            summary: {
                ...
                storageUsed: ((totalSize._sum.fileSize || 0) / constants_1.APP_CONSTANTS.UPLOAD.MAX_TOTAL_SIZE * 100).toFixed(1) + '%',
            },
            byType: filesByType,
            recentUploads,
        },

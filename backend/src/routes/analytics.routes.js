 ...
 *                   type: number
 */
router.get("/overview", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [totalBookings, completedBookings, pendingBookings, totalRevenue, avgBookingValue, topProperties, bookingTrends, revenueByMonth,] = yield Promise.all([
        // Total bookings
        ...
        // Completed bookings
        server_1.prisma.booking.count({
            where: Object.assign(Object.assign({}, baseWhere), { status: client_1.BookingStatus.COMPLETED }),
        }),
        // Pending bookings
        server_1.prisma.booking.count({
            where: Object.assign(Object.assign({}, baseWhere), { status: client_1.BookingStatus.PENDING }),
        }),
        // Total revenue
        server_1.prisma.booking.aggregate({
            where: Object.assign(Object.assign({}, baseWhere), { paymentStatus: "PAID" }),
            _sum: { total: true },
        }),
    ...
    const propertyDetails = yield server_1.prisma.property.findMany({
    ...
    const previousPeriodData = yield server_1.prisma.booking.aggregate({
...
router.get("/bookings", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [bookingsByStatus, bookingsByProperty, bookingsByType, avgStayDuration, occupancyRate, cancellationRate, bookingsBySource, peakTimes,] = yield Promise.all([
        // Bookings by status
        server_1.prisma.booking.groupBy({
            by: ["status"],
            where: baseWhere,
      ...
      `,
        // Cancellation rate
        server_1.prisma.booking.aggregate({
            where: Object.assign(Object.assign({}, baseWhere), { status: client_1.BookingStatus.CANCELLED }),
            _count: true,
        }),
    ...
    const cancelledBookings = ((_a = bookingsByStatus.find((s) => s.status === client_1.BookingStatus.CANCELLED)) === null || _a === void 0 ? void 0 : _a._count.status) || 0;
    const cancellationRatePercent = totalBookingsForRate > 0
...
router.get("/revenue", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [totalRevenue, revenueBreakdown, revenueByProperty, revenueByMonth, averageBookingValue, conversionRate,] = yield Promise.all([
        // Total revenue
        server_1.prisma.booking.aggregate({
            where: baseWhere,
            _sum: {
    ...
    const completedBookings = ((_a = conversionRate.find((s) => s.status === client_1.BookingStatus.COMPLETED)) === null || _a === void 0 ? void 0 : _a._count.status) || 0;
 ...
 */
router.get("/properties", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    (0, express_validator_1.query)("period")
        .optional()
        .isIn(["today", "week", "month", "quarter", "year"]),
], validate, (0, error_middleware_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [propertyStats, performanceMetrics, occupancyRates, avgRatings] = yield Promise.all([
        // Property statistics
        ...
        ${req.user.role === client_1.UserRole.ADMIN ? `WHERE p.host_id = '${req.user.id}'` : ""}
    ...
    const propertyPerformance = performanceMetrics.map((property) => {
        ...
        const avgRevenue = property.bookings.length > 0
 ...
 *         description: Internal server error
 */
router.get("/export", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    switch (type) {
        case "bookings":
            data = yield server_1.prisma.booking.findMany({
                where: baseWhere,
                include: {
                    property: {
        ...
        case "properties":
            ...
            data = yield server_1.prisma.property.findMany({
                where: propertyWhere,
                include: {
                    _count: {
                        select: {
    ...
    (0, logger_middleware_1.auditLog)("ANALYTICS_EXPORTED", req.user.id, {
        type,
        period,
        format,

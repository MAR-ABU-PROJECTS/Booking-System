...
export const validateBookingDates = (checkIn: Date, checkOut: Date) => {
  ...
  const minAdvance = new Date(
    now.getTime() + APP_CONSTANTS.BOOKING.MIN_ADVANCE_HOURS * 60 * 60 * 1000
  );
  const maxAdvance = new Date(
    now.getTime() + APP_CONSTANTS.BOOKING.MAX_ADVANCE_DAYS * 24 * 60 * 60 * 1000
  );
  ...
  if (checkIn < minAdvance) {
    errors.push(
      `Check-in must be at least ${APP_CONSTANTS.BOOKING.MIN_ADVANCE_HOURS} hours in advance`
    );
  }

  if (checkIn > maxAdvance) {
    errors.push(
      `Check-in cannot be more than ${APP_CONSTANTS.BOOKING.MAX_ADVANCE_DAYS} days in advance`
    );
  }
  ...
  if (nights < APP_CONSTANTS.BOOKING.MIN_STAY_NIGHTS) {
    errors.push(
      `Minimum stay is ${APP_CONSTANTS.BOOKING.MIN_STAY_NIGHTS} night(s)`
    );
  }

  if (nights > APP_CONSTANTS.BOOKING.MAX_STAY_NIGHTS) {
    errors.push(
      `Maximum stay is ${APP_CONSTANTS.BOOKING.MAX_STAY_NIGHTS} nights`
    );
  }

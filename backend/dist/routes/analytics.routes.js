...
router.get("/overview", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [totalBookings, completedBookings, pendingBookings, totalRevenue, avgBookingValue, topProperties, bookingTrends, revenueByMonth,] = await Promise.all([
        // Total bookings
    ...
    const previousPeriodData = await server_1.prisma.booking.aggregate({
        where: {
            createdAt: {
...
router.get("/bookings", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [bookingsByStatus, bookingsByProperty, bookingsByType, avgStayDuration, occupancyRate, cancellationRate, bookingsBySource, peakTimes,] = await Promise.all([
        // Bookings by status
        server_1.prisma.booking.groupBy({
            by: ["status"],
            where: baseWhere,
    ...
    const cancelledBookings = bookingsByStatus.find((s) => s.status === client_1.BookingStatus.CANCELLED)?._count
        .status || 0;
...
router.get("/revenue", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [totalRevenue, revenueBreakdown, revenueByProperty, revenueByMonth, averageBookingValue, conversionRate,] = await Promise.all([
        // Total revenue
        server_1.prisma.booking.aggregate({
            where: baseWhere,
            _sum: {
    ...
    const completedBookings = conversionRate.find((s) => s.status === client_1.BookingStatus.COMPLETED)?._count
        .status || 0;
...
router.get("/properties", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    const [propertyStats, performanceMetrics, occupancyRates, avgRatings] = await Promise.all([
        // Property statistics
...
router.get("/export", (0, authservice_1.requireAuth)({ role: client_1.UserRole.ADMIN }), [
    ...
    switch (type) {
        case "bookings":
            data = await server_1.prisma.booking.findMany({
                where: baseWhere,
                include: {
                    property: {
        ...
        case "properties":
            ...
            data = await server_1.prisma.property.findMany({
                where: propertyWhere,
                include: {
                    _count: {
                        select: {

 ...
 */
router.post("/upload-id", (0, authservice_1.requireAuth)(), fileservice_1.fileService.idDocumentUploader().fields([
    ...
    if (!files?.idDocumentFront || files.idDocumentFront.length === 0) {
        throw new error_middleware_2.AppError("Front side of ID document is required", 400, "ID_FRONT_REQUIRED");
    }
    ...
    await (0, logger_middleware_1.auditLog)("ID_DOCUMENT_UPLOADED", req.user.email, {
        idType,
        userId: req.user.userId,
    }, req.ip);
    res.json({
        success: true,
        message: "ID document uploaded successfully. Your identity has been verified.",
        data: {
